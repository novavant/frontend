...
export default function BankAdd() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 pb-36">
      <Head>
        <title>{applicationData?.name || 'Nova Vant'} | Tambah Rekening</title>
      </Head>
      ...
      <div className="sticky top-0 z-40 backdrop-blur-xl bg-slate-900/90 border-b border-slate-800">
        <div className="max-w-md mx-auto px-5 py-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-slate-400">Langkah {Math.min(step, 3)} dari 3</p>
          </div>
        </div>
        {/* Progress Bar */}
        ...
        {step === 4 ? (
          ...
          <>
            ...
            <div className="text-center mb-6">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-600 flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-600/30">
                <Icon icon="mdi:bank-plus" className="w-10 h-10 text-white" />
              </div>
              <h2 className="text-xl font-bold text-white mb-1">Rekening Baru</h2>
              <p className="text-slate-400 text-sm">Isi data rekening bank Anda</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="bg-slate-800 rounded-2xl p-4 border border-slate-700">
                <label className="flex items-center gap-2 text-white font-semibold mb-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${bankId ? 'bg-green-600 text-white' : 'bg-slate-600 text-slate-300'}`}>
                    {bankId ? <Icon icon="mdi:check" className="w-4 h-4" /> : '1'}
                  </div>
                  Pilih Bank
                </label>
                        ...
                        {banks
                          ...
                          .map(bank => (
                            <div
                              ...
                              onClick={() => { setBankId(bank.id.toString()); setStep(2); setShowBankDropdown(false); setBankSearch(''); }}
                            >
              ...
              {/* Account Number */}
              <div className={`bg-slate-800 rounded-2xl p-4 border transition-all ${bankId ? 'border-slate-700 opacity-100' : 'border-slate-800 opacity-50 pointer-events-none'}`}>
                <label className="flex items-center gap-2 text-white font-semibold mb-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${bankAccount ? 'bg-green-600 text-white' : 'bg-slate-600 text-slate-300'}`}>
                    {bankAccount ? <Icon icon="mdi:check" className="w-4 h-4" /> : '2'}
                  </div>
                  Nomor Rekening
                </label>
                <input
                  ...
                  value={bankAccount}
                  onChange={e => { setBankAccount(e.target.value.replace(/[^0-9]/g, '')); setStep(2); }}
              ...
              {/* Account Name */}
              <div className={`bg-slate-800 rounded-2xl p-4 border transition-all ${bankAccount ? 'border-slate-700 opacity-100' : 'border-slate-800 opacity-50 pointer-events-none'}`}>
                <label className="flex items-center gap-2 text-white font-semibold mb-3">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${fullName ? 'bg-green-600 text-white' : 'bg-slate-600 text-slate-300'}`}>
                    {fullName ? <Icon icon="mdi:check" className="w-4 h-4" /> : '3'}
                  </div>
                  Nama Pemilik
                </label>
                <input
                  ...
                  value={fullName}
                  onChange={e => { setFullName(e.target.value); setStep(3); }}
              ...
              <button
                ...
                className={`w-full py-4 rounded-xl text-white font-bold flex items-center justify-center gap-2 transition-all ${isSubmitting || !bankId || !bankAccount || !fullName
